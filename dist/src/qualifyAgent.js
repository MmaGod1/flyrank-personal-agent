...
function formatMatch(match) {
    ...
    if (match.experienceConcern)
        lines.push(`Experience concern: ${match.experienceConcern}`);
    if (match.locationConcern)
        lines.push(`Location concern: ${match.locationConcern}`);
    if (match.otherConcerns.length)
        lines.push(`Other concerns: ${match.otherConcerns.join("; ")}`);
 ...
 */
export async function* streamQualificationChunks(messages, apiKey, context) {
