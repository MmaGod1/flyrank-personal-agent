...
import { streamQualificationChunks, type ChatMessage, type ChatContext } from "./qualifyAgent.js";
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "POST" && url === "/api/qualify") {
    ...
    try {
      const body = (await readBody(request)) as { messages?: ChatMessage[]; jobId?: string };
      ...
      try {
        const [profile, cv, savedJobs] = await Promise.all([
          ...
          body.jobId ? readJson<SavedJob[]>(STORAGE_KEYS.savedJobs) : Promise.resolve(null),
        ]);
  ...
  if (url === "/api/profile") {
    if (request.method === "GET") {
      try {
        const stored = await readJson<UserProfile>(STORAGE_KEYS.profile);
    ...
    if (request.method === "POST") {
      try {
        const body = (await readBody(request)) as Partial<UserProfile>;
        const existing = (await readJson<UserProfile>(STORAGE_KEYS.profile)) ?? DEFAULT_PROFILE;
  ...
  if (url === "/api/cv") {
    ...
    if (request.method === "POST") {
      try {
        const body = (await readBody(request)) as { rawText?: string };
        ...
        const updated: CV = { rawText: body.rawText, updatedAt: new Date().toISOString() };
  ...
  if (url === "/api/jobs") {
    ...
    if (request.method === "POST") {
      try {
        const body = (await readBody(request)) as { job?: Job; match?: JobMatch };
  ...
  if (request.method === "POST" && url === "/api/tailor") {
    ...
    try {
      const body = (await readBody(request)) as { jobId?: string };
      if (!body.jobId) {
      ...
      const [cv, savedJobs] = await Promise.all([
        readJson<CV>(STORAGE_KEYS.cv),
        readJson<SavedJob[]>(STORAGE_KEYS.savedJobs),
      ]);
      ...
      const existingDrafts = (await readJson<TailoredCV[]>(STORAGE_KEYS.tailoredCvs)) ?? [];
