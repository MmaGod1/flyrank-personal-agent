 ...
 */
export function renderMessageBubble(part: ChatMessagePart): RenderedBubble {
  switch (part.kind) {
    case "user":
...
export type SendButtonState = "idle" | "sending";

export interface SendButtonView {
  ariaLabel: string;
  iconKey: "send" | "stop";
  isStopping: boolean;
}
 ...
 */
export function sendButtonView(state: SendButtonState): SendButtonView {
  if (state === "sending") {
